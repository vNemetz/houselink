        ...
        while True:
            ...
            if check_reed_switch():
                ...
                while not check_reed_switch_2():
                    time.sleep(0.1)
                set_motor("stop", 0)
                current_state = "locked"  # Update state to locked after auto-close
                break
...
def control_lock():
    ...
    try:
        ...
        if command == 'unlock' and current_state == 'locked':
            if motor_action("forward", 3):  # 3 seconds or until reed switch activates # Wait for 15 seconds before auto-closing
                motor_action("backward", 3)  # Auto-close the motor
                current_state = 'locked'  # Set state to locked after auto-close
                logger.info("Successfully auto-closed and locked")
                return jsonify({'status': 'Success', 'action': 'auto-closed and locked'})
            else:
